<template>
    <div :style="{marginTop:appMarginTop}" v-if="isReady">
        <div class="mcht-data-head">
            ...
            <div class="mcht-data-head-title">沉淀资金统计</div>
        </div>
        <!-- 选择地址 -->
...
export default {
    components: {
        ...
        WisdomNav: () => import('@/components/wisdomNav'),
        List
    },
    data() {
        return {
            ...
            appMarginTop: '20px',
            columns: [
                {name: '机构名称', key: 'simple_name'},
                {name: '商户资金 (万元)', key: 'merchants'},
                {name: '代发资金 (万元)', key: 'undertakes'},
                {name: '对公资金 (万元)', key: 'contrary'},
                {name: '获客资金 (万元)', key: 'guest'},
                {name: '小计 (万元)', key: 'subtotal', color: '#EE7F61'}
            ],
            rows: [],
            loading: false,
            finished: false,
    ...
    methods: {
        ...
        onLoad() {
            ...
            Util.axiosPost('v1/scenario/deposit', params).then(async res => {
                if(res.status == 200) {
