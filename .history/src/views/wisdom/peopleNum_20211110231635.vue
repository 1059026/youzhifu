<template>
    <div :style="{marginTop:appMarginTop}" v-if="isReady">
        <div class="mcht-data-head">
            ...
            <div class="mcht-data-head-title">获客统计</div>
        </div>
        <!-- 选择地址 -->
...
export default {
    ...
    data() {
        return {
            ...
            columns: [
                ...
                {name: '场景数', key: 'scen_num'},
                {name: '代发数', key: 'undertakes'},
                {name: '对公数', key: 'contrary'},
                {name: '获客数', key: 'guest'},
                {name: '小计', key: 'subtotal', color: '#EE7F61'}
            ],
            rows: [],
            loading: false,
            finished: false,
    ...
    methods: {
        ...
        onLoad() {
            ...
            Util.axiosPost('v1/scenario/guest', params).then(async res => {
                if(res.status == 200) {
